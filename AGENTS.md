...
| **生理需求** | 精力/饱腹当前值与上限、睡眠债/断食债、食物刷新与背包上限 | agent 不能改公式，只能用 SLEEP / USE 食物回应 |
...
| **探索兴趣** | 想去地图哪块 | 走寻路接口 |
| **何时睡觉/进食** | 何时 SLEEP、捡/吃/送野果、是否喂别人 | 受精力/饱腹条、食物上限与原语约束 |
...
- 食物背包上限与地面野果刷新走配置；agent 只决定何时捡、吃、送、睡。
...
| **世界续局** | ✅ 单槽 `data/saves/world.json` | 关闭/重开接续世界（tick、位置、迷雾、地面物品、地形覆盖、背包、精力/饱腹与上限、事件进度、计划、性格漂移、未醒完的 `sleep_until_tick`、观察者 G/选中）。记忆/关系仍走各自 JSON。Ctrl+R / `tools/reset_game.py` 删存档开新局。不存进行中的行走路径、WAIT 剩余、inflight LLM。 |
