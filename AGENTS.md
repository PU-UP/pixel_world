...
决策 prompt 优先列出**已探索区域边缘的未探索可走格**（按方向取样、须 A* 可达），邻格只作微调。观察陈述已探索格数、未探索方向坐标、以及在熟悉区域的停留 tick。不注入「去远方」之类假意愿。
...
- **黎明结算**（`day_index` 增加时）：
  ...
  - **健康**按更新后的连续缺觉/断食天数结算：缺则扣、两项都缺再加罚；睡够且吃过则恢复，只做到一件则少恢复。健康只在黎明变，不每 tick 扣。完全忽视约 4 个黎明可降至 0。
