...
##
## Tick 驱动 LLM 决策 → executor → player
##
...
func setup(player: Player, clock: GameClock, llm: LlmClientScript, logger: LoggerScript, persona: PersonaScript) -> void:
	_player = player
	_clock = clock
	_llm = llm
	_logger = logger
...
func _on_llm_completed(_request_id: int, body: Dictionary, meta: Dictionary) -> void:
	...
	decision_made.emit(int(meta.get("tick", -1)), _last_raw, _last_action, result)
